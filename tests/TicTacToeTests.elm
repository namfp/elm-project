...
--
--            game =
--                { board = board, player = TicTacToe.Self }
--
--            reference =
...
--            nextBoard =
--                board
--                    |> (\x -> { board = x, player = TicTacToe.Self })
...
--                    ]
--          in
--          test "should find one correct move for SELF" (\_ -> Expect.equal nextGame { score = 1, originalGame = game, nextMove = Just { board = referenceBoard, player = TicTacToe.Opponent } })
--        , let
--            board =
--                Array.fromList
--                    [ Array.fromList [ xX, xX, oO ]
...
--                    ]
--          in
--          test "should find one correct move for diagonal" (\_ -> Expect.equal nextGame { score = 1, originalGame = game, nextMove = Just { board = referenceBoard, player = TicTacToe.Opponent } })
        ...
        let
          ...
          in
          test "should avoid loss" (\_ -> Expect.equal nextGame { score = 1, originalGame = game, nextMove = Just { board = referenceBoard, player = TicTacToe.Opponent } })
        ]
